...
async def get_task(task_id):
    return await tasks.find_one({"task_id": task_id})
...
async def update_many(task_id, data):
    await tasks.update_one(
        {"task_id": task_id},
        {"$set": data}
    )
...
async def increase_forwarded(task_id):
    await tasks.update_one({"task_id": task_id}, {"$inc": {"forwarded": 1}})

async def increase_skipped(task_id):
    await tasks.update_one({"task_id": task_id}, {"$inc": {"skipped": 1}})

async def increase_deleted(task_id):
    await tasks.update_one({"task_id": task_id}, {"$inc": {"deleted": 1}})

async def increase_duplicate(task_id):
    await tasks.update_one({"task_id": task_id}, {"$inc": {"duplicate": 1}})

async def increase_filtered(task_id):
    await tasks.update_one({"task_id": task_id}, {"$inc": {"filtered": 1}})
